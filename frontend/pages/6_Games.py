...
brief, board = st.columns([1, 2], gap="large")

with brief:
    st.markdown("#### Objective")
    st.info(level["prompt"])
    ...
    if level.get("epsilon") is not None:
        rules.append(f"Accuracy tolerance: {level['epsilon']}")
    ...
    if level["attempts"]:
        st.metric("Your best", f"{level['best_score']:.2f}",
                  "passed" if level["passed"] else "not yet")

with board:
    st.markdown("#### Build your circuit")
    ir = composer.render("game", secondary=False, allowed_gates=level.get("allowed_gates"))
    ...
    if st.button("▶ Run & score", type="primary", use_container_width=True):
        try:
            ...
            for _ in range(60):
                ...
                time.sleep(0.5)
            placeholder.empty()
            st.session_state["game_attempt"] = outcome
            st.rerun()
        except ApiError as exc:
            st.error(str(exc))
